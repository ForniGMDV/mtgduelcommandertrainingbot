...
function initializeDeck(deckCards: Card[]): { commander: Card | null; hand: Card[]; library: Card[] } {
...
export async function createGame(playerDeck?: Card[], opponentDeck?: Card[]): Promise<GameState> {
  const gameId = `game_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`

  const pDeck = playerDeck && playerDeck.length > 0 ? playerDeck : getRandomCards(100)
  
  const oDeck = await Promise.all(
    SLIMEFOOT_DECK_LIST.map((name, idx) => buildCard(name, `bot_card_${idx}`))
  )

  const playerSetup = initializeDeck(pDeck)
  const opponentSetup = initializeDeck(oDeck)

  const opponentBattlefield: Card[] = []
...
export function playCard(gameId: string, cardId: string): GameState | null {
...
export function drawCard(gameId: string, playerId: string): GameState | null {
  const game = GAME_STATES.get(gameId)
  if (!game) return null

  const player = playerId === 'player_1' ? game.player : game.opponent
  if (player.library.length === 0) return game
...
export function shuffleLibrary(gameId: string, playerId: string): GameState | null {
  const game = GAME_STATES.get(gameId)
  if (!game) return null
...
export function moveCardToZone(
  gameId: string,
  cardId: string,
  fromZone: 'hand' | 'battlefield' | 'library' | 'graveyard' | 'exile',
  toZone: 'hand' | 'battlefield' | 'library' | 'graveyard' | 'exile',
  playerId: string
): GameState | null {
  const game = GAME_STATES.get(gameId)
  if (!game) return null
  ...
  const player = playerId === 'player_1' ? game.player : game.opponent
  const fromArray = player[fromZone] as Card[]
  const toArray = player[toZone] as Card[]
...
function botPlayCards(gameId: string): void {
  const game = GAME_STATES.get(gameId)
  if (!game) return
...
function botTakeTurn(gameId: string): void {
  const game = GAME_STATES.get(gameId)
  if (!game) return
...
export function endTurn(gameId: string): GameState | null {
  const game = GAME_STATES.get(gameId)
  if (!game) return null
